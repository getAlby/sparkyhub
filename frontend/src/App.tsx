  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      ...
      <Routes>
        ...
        <Route path="/signup" element={token ? <Navigate to="/" /> : <SignupPage setToken={handleSetToken} />} />
        ...
        >
          ...
          <Route index element={
            <div className="w-full max-w-4xl px-4"> {/* Added container for content */}
              {/* Removed welcome message, AppsManager is the main content now */}
              {/* <p>
                 Token: <code>{token}</code>
               </p> */}
              <div className="flex justify-end mb-4"> {/* Position logout button */}
                <button onClick={handleLogout} className="text-sm underline">Logout</button>
              </div>
              {token && <AppsManager token={token} />} {/* Render AppsManager only if token exists */}
            </div>
          } />
